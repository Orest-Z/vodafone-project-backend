...
public interface GamePlayRepository extends JpaRepository<GamePlay, UUID> {
    ...
    @Query("""
        SELECT gp FROM GamePlay gp
        WHERE gp.tourist.id = :touristId
        AND gp.prize.prizeType = al.vodafone.vodafone_project_backend.model.PrizeType.PACK_DISCOUNT
        AND gp.redeemedAt IS NULL
